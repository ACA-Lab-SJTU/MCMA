...
def Error_of_Approximator(A,x,y): #Input one Approximator, x, y. Return  errorA[lenN]
    ...
    for i in range(len(y)):
        errorA.append(error_measure(y[i],predictA[i]))
    return errorA
...
def Label_for_C_competive(errorA,eb): #errorA[numA][lenN] return labelA[lenN][numA+1] for training C
    labelA=[]
    for i in range(lenN):
        labelA.append(numA)
        MINERROR=100
        for j in range(numA):
            if (errorA[j][i]<=eb and errorA[j][i]<MINERROR):
                MINERROR=errorA[j][i]
                labelA[i]=j
    labelA=keras.utils.to_categorical(labelA,numA+1)
    return labelA
...
def evaluate(A, C, x_test, y_test, error_bound):
    ...
    invocation_of_A0 =float(sum([1 if (v==0) else 0 for v in predictC])) / float(1e-10 + lenN)
    invocation_of_A1 =float(sum([1 if (v==1) else 0 for v in predictC])) / float(1e-10 + lenN)
    ...
    return{'accuracy_of_C':accuracy_of_C,
          ...
          'invocation_of_A0':invocation_of_A0,
          'invocation_of_A1':invocation_of_A1}
...
#    return model
def AcceleratorModel_MCMAD(net_list,type=0):
    if (len(net_list)<2):
        print('Error! input accelerator net structure is wrong!')
        exit(0)
    model=Sequential()
    if (type==1 and (len(net_list)==3)):
        model.add(Dense(net_list[1]*2,input_shape=(net_list[0],)))
    else:
        model.add(Dense(net_list[1],input_shape=(net_list[0],)))
    model.add(Activation('sigmoid'))
    
    for i in range(2,len(net_list)):
        if ((type==1) and (len(net_list)==4) and (i==2)):
            model.add(Dense(net_list[i]*2))
        else:
...
def training(eb):
    ...
    for index in range(iteration):
        ...
        print("The training data in index: ",index)
        for j in range(numA):
            print ("Approximator:",j,x_train[j].shape,y_train[j].shape)
        ...
        if (method=='MCMA_competive'):
            labelA_trainC=Label_for_C_competive(errorA,eb)
        else:
...
def main(app,meth,eA,eC):
    ...
    eb_using=0
    if (app=="sobel" or app=="fft" or app=="kmeans"):
        eb_using=eb[1]
    if (app=="jpeg" or app=="jmeint"):
        eb_using=eb[0]
    if (app=="inversek2j"):
        eb_using=0.03839
    if (app=="blackscholes"):
